...
Pilot disambiguation (n = round(raw_ns − onset_diff_ns) / 7 ms)
        ↓
TDOA = sync_delta_A − sync_delta_B − n×7 ms + path_correction
```
...
(c × distance), stable to better than 1 ns over any measurement interval.
...
```
timeNs = anchor_wall_ns + (extended_first − anchor_sample_num) × 1e9 / outputSampleRate
```

| Source | Magnitude | Character |
|--------|-----------|-----------|
| NTP quality at anchor time | ±50–200 µs | One-time systematic per process run |
| TCXO drift (5 ppm) | 0.15 µs/30 s, 9 µs/30 min | Accumulates monotonically since anchor |
| Two-node onset_diff error | ~100–400 µs combined | What matters for disambiguation |

**For disambiguation:** requires `|error| < 3.5 ms`. Hardware timestamps deliver
~100–400 µs — correct disambiguation essentially 100% of the time.
...
**Without Part A (fallback `time.time_ns()` in Python):** ±50–500 µs RMS per buffer,
potentially seconds of error during buffer backlog. This caused occasional wrong-`n`
selections and ±7 ms TDOA catastrophes — the dominant source of the pre-Part A ~230 µs
...
2. Cross-correlate with complex exponential template:
   `corr = Σ(audio × conj(template))` — result is a complex number
...
| FM pilot SNR | Phase uncertainty | Timing σ |
|---|---|---|
...
**Variance of `sync_sample`: ~0.1–1 µs RMS — not the bottleneck.**
...
The pipeline converts from target-domain (64 kHz) → raw ADC (×32) → sync-domain (÷8)
...
- Fast, clean onset: ~0–100 µs (correlated quantisation)
- Gradual or borderline onset: ±1 ms (one-window shift between nodes)
- Mean in practice: likely 100–500 µs for typical LMR
...
one smoothing window (±125 µs at 64 kHz / 16-sample kernel) of the true PA shutoff —
approximately **8× better** than the previous threshold-crossing window centre
(±500 µs).
...
- **PA shutoff** (offset): the power amplifier bias is cut electronically — a nearly
  instantaneous event (sub-µs). The derivative of the received power envelope has a
...
```python
sync_delta_ns = (target_sample − sync_sample) × 1e9 / corrected_rate
```
...
| Pilot sync timing | ~0.1–1 µs | Negligible |
...
```python
raw_event_sample = m.target_sample × _sync_dec_factor          # → raw ADC domain
onset_offset_ns  = raw_event_sample × 1e9 / sample_rate_hz     # within-buffer offset
...
```python
T_sync = onset_time_ns − sync_delta_ns − dist(sync_tx, node) / c × 1e9
```
...
With hardware timestamps, `T_sync` accuracy is ~100–400 µs per node — well within the
...
```
raw_ns = sync_delta_A − sync_delta_B = true_TDOA + n × T_sync
```
...
```
|true_TDOA| ≤ dist(A, B) / c ≤ 100 km / (3×10⁸ m/s) ≈ 333 µs
```

After applying the known sync-tx path correction (which itself is ≤ 333 µs for 100 km
sync-tx range), the corrected value lies within:

```
|raw_ns + correction_ns| = |true_TDOA + n × T_sync|
```

Since `|true_TDOA| ≤ 333 µs << T_sync/2 = 3500 µs`, rounding to the nearest multiple
...
The 100 km constraint is a design parameter.  At 200 km the bound becomes 667 µs,
still well within T_sync/2 = 3500 µs — the method is valid to at least 1000 km
(3333 µs < 3500 µs).  The bound fails only beyond ~1050 km, far outside any realistic
...
```python
tdoa_ns = (sync_delta_A − sync_delta_B) − n × 7_000_000 + path_correction_ns
```
...
| Per-buffer NTP jitter on `onset_time_ns` | Part A: TCXO hardware timestamps in SoapySDRPlay3 | onset_diff error 50–200 µs → correct disambiguation |
...
### 1. ~~Use derivative peak for `sample_index` on offset events~~ ✓ Done
...
offset `sample_index` is now within ±125 µs of the true PA shutoff (vs. ±500 µs
threshold window), an ~8× improvement.  This will matter if disambiguation via
...
- Precision: typically ±5–50 µs depending on bandwidth, SNR, and snippet length
...
**Estimated improvement: ±500 µs → ±5–50 µs.**
**Complexity: low — xcorr already works; needs server integration with sync_delta result.**

### 3. ~~Smaller detection windows~~ — Deferred

Halving `window_samples` from 64 to 32 (at 64 kHz) reduces quantisation from ±500 µs
to ±250 µs, at the cost of increased susceptibility to brief noise spikes.
Requires increasing `min_hold_windows` to compensate.
`window_samples = 16` at 64 kHz → ±125 µs, but needs `min_hold_windows ≥ 4` for
debounce — same net latency, but finer resolution.

**Estimated improvement: ±500 µs → ±62–125 µs (with compensating debounce).**
...
Xcorr already achieves sub-µs offset precision and handles 27/34 offset pairs in
...
1 window = 64/256 000 = **250 µs** → halved quantisation.
With 16-sample windows: **62.5 µs**. Trade-off: 4× CPU.
...
across 68 real pairs is ~39 ms, so 0.2 s (±100 ms half-window) provides a 2.5× safety
margin for typical internet NTP.

Further tightening to ~0.005 s (±2.5 ms) would only be practical with PPS-disciplined
NTP (GPS 1PPS), which reduces absolute NTP error to <10 µs.  No nodes in the currently
...
| Event type | Mean (µs) | Std (µs) | Min (µs) | Max (µs) | Notes |
|------------|-----------|----------|----------|----------|-------|
| Onset | +188 | 1989 | −3394 | +3442 | Uniform across ±3.5 ms pilot period |
| Offset | +719 | 2211 | −3238 | +3270 | Uniform across ±3.5 ms pilot period |

Std ≈ 2020 µs matches the theoretical std of a uniform distribution on [−3.5, 3.5] ms
(7 ms / √12 ≈ 2020 µs). The nodes almost always used different pilot periods as their
...
limitation is moot for the offset measurement path (std = 0.93 µs confirmed above).

### xcorr lag (cross_correlate_snippets on co-located pairs)

| Event type | n pairs (good SNR) | Mean (µs) | Std (µs) | Min (µs) | Max (µs) |
|------------|-------------------|-----------|----------|----------|----------|
| Onset | 34 | −4.3 | 299 | −854 | +603 |
| Offset | 27 | −0.5 | **0.9** | −3.0 | +1.3 |

- **Offset xcorr: std = 0.9 µs** — sub-µs TDOA precision confirmed on real hardware data.
  The PA shutoff is a hard electronic edge; the xcorr finds it to within a few µs.
- Onset xcorr: std = 299 µs — wide scatter due to gradual carrier ramp-up.
...
|--------|---------|--------|------|
| xcorr plumbing (offset) | **Done — 0.9 µs** | std ≈ 0.9 µs | ~2400× vs. sync_delta |
| Derivative peak → sample_index | **Done — ±125 µs (onset_time_ns)** | ~200–500 ns | ~8× vs. window centre |
| Geometric pilot disambiguation | **Done** | removes pilot period ambiguity | no NTP required |
| Lower correlation_window_s default | **Done — 0.2 s** (was 0.5 s) | 2.5× safety margin over observed max | operational hygiene |
| Smaller windows (÷2) | ~1 ms | ~500 µs | 2× |
...
| Event type | n pairs (xcorr) | n pairs (sync_delta fallback) | xcorr mean (µs) | xcorr std (µs) | xcorr range (µs) |
|------------|----------------|-------------------------------|-----------------|----------------|------------------|
...
their sync_delta fallback gives mean=−1511 µs, std=2339 µs (random within ±3.5 ms pilot period).

### Before-vs-after comparison

| Event type | Method | Std (µs) | Improvement |
|------------|--------|----------|-------------|
| Onset | sync_delta (before) | 1989 | — |
| Onset | xcorr (after) | 299 | 6.6× |
| Offset | sync_delta (before) | 2211 | — |
| Offset | xcorr — good pairs (after) | **0.93** | **2376×** |

Offset xcorr precision is now **sub-µs** on real co-located hardware data.  For distributed
nodes, this translates directly to sub-µs TDOA precision — approximately ±280 m position
uncertainty per pair (c × 0.93 µs ≈ 279 m).

Onset xcorr (std=299 µs) is better than sync_delta but still wide because gradual carrier
...
| Event type | n good pairs | xcorr std (µs) | vs. xcorr baseline |
|------------|-------------|----------------|-------------------|
...
| | Before | After |
|---|---|---|
| Offset `sample_index` error vs. true PA shutoff | ~±500 µs (threshold window) | ~±125 µs (16-sample derivative kernel) |
...
|---|---|---|
| Baseline (sync_delta, no disambiguation) | 2211 µs | 1989 µs |
| + xcorr as primary TDOA method | **0.93 µs** | 299 µs |
| + derivative peak sample_index | **0.93 µs** (unchanged) | 299 µs (unchanged) |

The derivative peak improvement prepares `onset_time_ns` for future disambiguation use;
it does not further reduce xcorr TDOA noise, which is already at the sub-µs floor set
...
- **Pre-Part A observed std:** ~230 µs (dominated by wrong-`n` disambiguation outliers)
- **Expected post-Part A std:** limited by carrier detection quantisation, expected
  0–500 µs depending on signal quality — requires clean data (no backlogs) to measure
- **Target:** ±50 µs or better (achievable with derivative-based timing or xcorr)
...
The tone is phase-coherent during a transmission (EIA/TIA-603 defines a 180° "reverse
...
and if phase can be measured to 1–2°, that would give ±11–22 µs timing within the period.
...
- Production LMR PA fall times (VHF/UHF LDMOS/BJT): **1–10 µs** for the RF envelope
...
- Research on GaN PAs shows sub-200 ns fall times are achievable; production LMR is
  typically 1–5 µs but still a hard, consistent edge.

This is **10–100× sharper than onset**, and is already partially exploited:
...
**±100–500 ns** is achievable. At the current 64 kHz / 1 ms window: ±500 µs.
...
|----------|-------------|-----------|-----------|----------------------|
| P25 Ph1 (C4FM) | 4800 sym/s | 12.5 kHz | 48 sym / 10 ms | ~10–20 µs |
| P25 Ph2 (H-DQPSK) | 6000 sym/s | 12.5 kHz | 24 sym / 4 ms | ~10–20 µs |
| DMR (4-FSK TDMA) | 4800 sym/s | 12.5 kHz | 24 sym / 5 ms | ~10–20 µs |
| NXDN 6.25 kHz | 2400 sym/s | 6.25 kHz | 18 sym / 7.5 ms | ~20–40 µs |
| TETRA (Pi/4-DQPSK) | 18 000 sym/s | 25 kHz | — | ~5–10 µs |

For P25 and DMR, which are common in North American public safety LMR, matched-filter
detection of the frame sync word would provide **~10–20 µs** onset timing — 50–100×
...
The CRFS rule of thumb: timing accuracy ≈ c / (4 × bandwidth). For 12.5 kHz NFM:
timing resolution floor ≈ 6 km — meaningless without structure. For 200 kHz WBFM:
≈ 375 m. For the 19 kHz pilot tone itself (near-CW): the pilot period (52.6 µs) is the
ambiguity, and phase measurement precision of 1–2° at 20 dB SNR gives ~150–300 ns.

NFM has no equivalent continuous subcarrier, so we rely entirely on the transient
features (PA shutoff, digital sync words) for precision. The 15× narrower bandwidth
means the envelope transition is the only timing information available, and it requires
high sample rates (≥2 MS/s) to resolve sub-µs features.
...
|--------|---------------------|----------|------------|--------|
| Xcorr (already implemented) | ~1 µs (offset), ~300 µs (onset) | All LMR | Low | **Done** |
| Offset-event derivative peak → `sample_index` | ~125 µs (onset_time_ns) | All LMR | Medium | **Done** |
| Geometric pilot disambiguation | removes ±3.5 ms alias | All LMR | Medium | **Done** |
| Lower `correlation_window_s` default (0.2 s) | 2.5× grouping safety margin | All LMR | Trivial | **Done** |
| Smaller detection windows | ~250 µs (sync_delta fallback only) | All LMR | Low (config + recapture) | **Deferred** |
| P25 / DMR sync word matched filter | ~10–20 µs | Digital LMR | High (new detector) | Not started |
