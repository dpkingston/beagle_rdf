...
TDOA geolocation requires knowing, for each node, *when* the target signal arrived relative to a common reference. The naive approach — compare absolute GPS-disciplined timestamps across nodes — requires microsecond-level absolute clock synchronization. GPS-disciplined chrony can achieve ~1 µs, but USB jitter and kernel scheduling can degrade this significantly for SDR captures.
...
```
sync_delta_ns = (N_target − N_sync) × 1_000_000_000 / sample_rate_hz
```
...
RTL-SDR crystal accuracy is typically 50–100 ppm uncalibrated. Over a 200 ms sync window, this introduces 10–20 µs of timing error. The FM stereo pilot eliminates this error.
...
```
expected_phase_advance = 2π × 19000 × 0.010 = 380π rad per 10 ms window
...
```
sync_delta_ns_corrected = sync_delta_ns × correction_factor
```

After calibration, crystal error is reduced to <1 ppm, cutting timing error to <0.2 µs over 200 ms.
...
**Solution: GPS 1PPS RF injection.** The GPS module's 1PPS output is attenuated (via 10 MΩ resistors) and injected into both SDR antenna inputs. The pulse appears as a broadband amplitude spike in both IQ streams at the same physical instant.
...
sync_delta_ns  = (t_target_onset − t_sync_event) × 1e9
```

This eliminates USB jitter from the measurement. The only sources of error are GPS 1PPS jitter (<100 ns) and spike detection accuracy (~0.5 µs at 2 MSPS).

**Hardware for 1PPS injection:**

```
GPS 1PPS (3.3V TTL) ──── 10 MΩ resistor ──── SMA tee ──── SDR antenna input
                                                    └─────── antenna (coax)
```

One 10 MΩ resistor per SDR. Signal level at SDR input: ≈ −82 dBm (38 dB above noise floor, well below saturation). Effect on receiver sensitivity: negligible. Total cost: ~$5.
...
|-------------|-----------|-------------|
| Crystal rate (100 ppm, 200 ms) | 20 µs raw | FM pilot crystal calibration → <0.2 µs |
| Pilot phase measurement | <1 µs | — |
...
| Total after calibration | ~1–5 µs | — |

At 5 µs: position uncertainty ≈ 1.5 km, meets the neighborhood-level goal.
...
| **Total** | **~1–2 µs → ~300–600 m position** |
