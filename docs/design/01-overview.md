...
The path-delay correction accounts for the FM station's signal arriving at different times at each node (due to different distances). With FCC-documented transmitter coordinates (accuracy ~100 m), this correction is accurate to <1 µs across Seattle metro.
...
| SMA tees + 10 MΩ resistors | GPS 1PPS injection into SDR inputs |
...
|----------|-----------------|---------------------|
| `freq_hop` (single RTL-SDR) | ~1–5 µs | ~300 m–1.5 km |
| `rspduo` (SDRplay RSPduo) | ~1–2 µs | ~300–600 m |
| `two_sdr` + GPS 1PPS injection | <1 µs | ~300 m |
| `single_sdr` (wideband) | <1 µs | ~300 m |
