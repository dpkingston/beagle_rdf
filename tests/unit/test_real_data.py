...
class TestRealDataPilotDisambiguation:
    ...
    Fixture pair indices identified from raw sync_delta analysis:
      onset  n=0:  pair 4  (raw=  −166 µs)
      onset  n=+1: pair 1  (raw=+5974 µs), pair 13 (+3522 µs),
                   pair 14 (+4582 µs), pair 17 (+4534 µs)
      onset  n=−1: pair 7  (raw=−4130 µs), pair 21 (−3826 µs),
                   pair 31 (−4846 µs)
      offset n=0:  pair 34 (raw=  −318 µs)
      offset n=+1: pair 3  (raw=+4610 µs), pair 8  (+3966 µs),
                   pair 10 (+5054 µs)
      offset n=−1: pair 0  (raw=−4366 µs), pair 15 (−5766 µs),
                   pair 25 (−3526 µs)
    """
    ...
    def test_n_zero_onset(self):
        """onset pair 4: raw=−166 µs → n=0, TDOA unchanged."""
        fx = _load_fixtures("onset")[4]
        raw_us = (fx["sync_delta_ns_a"] - fx["sync_delta_ns_b"]) / 1e3
        assert abs(raw_us) < 3500, f"Precondition: raw={raw_us:.0f} µs should be n=0"
        tdoa = self._tdoa_no_snippet(fx)
        assert tdoa is not None
        assert abs(tdoa) < _T_SYNC_S / 2

    def test_n_zero_offset(self):
        """offset pair 34: raw=−318 µs → n=0, TDOA unchanged."""
    ...
    def test_n_plus_one_onset_pairs(self):
        ...
        for idx in (1, 13, 14, 17):
            fx = pairs[idx]
            raw_us = (fx["sync_delta_ns_a"] - fx["sync_delta_ns_b"]) / 1e3
            assert raw_us > 3500, f"Precondition: pair {idx} raw={raw_us:.0f} µs should be n=+1"
            tdoa = self._tdoa_no_snippet(fx)
            ...
            assert abs(tdoa) < _T_SYNC_S / 2, (
                ...
                f"after disambiguation (raw={raw_us:.0f} µs)"
            )
